...
public class MarineTargetZombieSystem : JobComponentSystem {
    ...
    private struct Job : IJobForEachWithEntity<Translation, Skeleton_PlayAnim, HasTarget, MarineShoot> {

        public float time;
        public float deltaTime;
        public NativeQueue<MarineShotZombieAction>.Concurrent queuedActions;
    ...
    protected override JobHandle OnUpdate(JobHandle inputDeps) {
        Job job = new Job {
            time = Time.time,
            deltaTime = Time.deltaTime,
            queuedActions = TestECS.queuedActions.ToConcurrent()
        };
        return job.Schedule(this, inputDeps);
    }
